...
extern "C" fn init() {
    ...
    let game_state = GameState {
        ...
        first_player: if rand::thread_rng().gen_bool(0.5) { Player::User } else { Player::Program },
        winner: None,
    };
    ...
    match action {
        PebblesAction::Turn(pebbles) => {
            ...
            if game_state.pebbles_remaining == 0 {
                ...
                let program_pebbles = (rand::thread_rng().gen_range(1..=game_state.max_pebbles_per_turn)).min(game_state.pebbles_remaining);
        ...
        PebblesAction::Restart { difficulty, pebbles_count, max_pebbles_per_turn } => {
            ...
            game_state.first_player = if rand::thread_rng().gen_bool(0.5) { Player::User } else { Player::Program };
